...
class dataset(Dataset):
    def __init__(self, args, train=True, trainlist=None, testlist=None):
        ...
        self.feature_path = args.feature_dir_ucf

        self.trainlist = self.txt2list(
            txtpath=os.path.join(self.dataset_path, self.dataset_name, 'train_split_10crop.txt'))
        self.testlist = self.txt2list(
            txtpath=os.path.join(self.dataset_path, self.dataset_name, 'test_split_10crop.txt'))
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            feature = np.load(file=os.path.join(self.feature_path, data_video_name + '.npy'))
